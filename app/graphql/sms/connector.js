...
class LaunchConnector {
   ...
   */
  async verifySend(area, mobile) {
    try {
      await this.service.sms.verifySend(mobile, area);
   ...
   */
  async mobileCodeLogin(area, mobile, code) {
    try {
      const result = await this.service.sms.verifyCheck(area, mobile, code);
      if (result) {
        const user = await this.service.user.findUserByMobile(area, mobile);
        ...
        if (!user) {
          ...
          const userinfo = { area, mobile, password, avatar };
